Yondu Laravel Couchbase
===============

For Laravel Eloquent and Query builder that support Couchbase.
...
| Couchbase | Laravel | Php |
| --- | --- | --- |
| ^7.x | 11.x | ^8.0 |

Make sure you have the Couchbase PHP driver installed. You can find installation instructions at https://docs.couchbase.com/php-sdk/current/hello-world/start-using-sdk.html

Installation using composer:

```
composer require yondu/couchbase
```

And add the service provider in `config/app.php`:

```php
Yondu\CouchbaseServiceProvider::class,
```
...
```php
$app->register(Yondu\CouchbaseServiceProvider::class);
...
use Yondu\Eloquent\Model as Eloquent;
...
- Create a bucket
```bash
php artisan couchbase:bucket:create --bucket DBname
```
...
```php
'CouchbaseModel'       => 'Yondu\Eloquent\Model',
```
...
```php
use Yondu\Eloquent\SoftDeletes;
...
If you're using a hybrid Couchbase and SQL setup, you're in luck! The model will automatically return a Couchbase- or SQL-relation based on the type of the related model. Of course, if you want this functionality to work both ways, your SQL-Models will need use the `Yondu\Eloquent\HybridRelations` trait. Note that this functionality only works for hasOne, hasMany and belongsTo relations.

Example SQL-based User model:

```php
use Yondu\Eloquent\HybridRelations;
